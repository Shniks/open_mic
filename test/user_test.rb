...
class UserTest < Minitest::Test
  ...
  def test_user_can_have_jokes_initially
    sal = User.new("Sal")
  ...
  def test_user_can_learn_jokes
    ...
    assert_equal joke, sal.jokes.last
  end
  ...
  def test_user_can_tell_another_user_a_joke
    ...
    assert_equal joke, ali.jokes.last
  end

  def test_count_of_users_jokes
    joke = Joke.new({id: 1, question: "Why did the strawberry cross the road?", answer: "Because his mother was in a jam."})
    sal = User.new("Sal")
    ali = User.new("Ali")
    sal.tell(ali, joke)
  ...
  def test_allow_user_to_perform_routines
